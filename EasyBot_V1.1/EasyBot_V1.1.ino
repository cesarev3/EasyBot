...
void loop() {
  ...
  saida = digitalRead(botaoVermelho);
...
//  distancias[i] = distancia;
 ...
 // distancia1  = ( distancias[0]   + distancias[1]  + distancias[2]
 //   + distancias[3]  + distancias[4]) / 5;
...
void termometro() { // Leitura do termometro
  ...
  temperaturas[temperaturaLida] = (500 * adcLido) / 1023; // equiparar multimetro com prototipo
  //  distancias[temperaturaLida] = (adcLido * ((1.1/1023)*100)) - 0.5; // equiparar multimetro com projeto caixa
...
void luzdisplay() { //luz do display
  ...
  if(ldrValor > 600 && ldrOn == 1) {
    ...
    ledPWM = map(ldrValor,70,599,6,255);
  }
...
void imprimindoDistancia () {
  ...
  lcd.print("Traseira:      ");
  lcd.setCursor(11,1);
  lcd.print(distancia1 + 14);
  lcd.setCursor(14,1);
  lcd.print("cm");
}

void imprimindoVoltagem () {
  lcd.setCursor(0,0);
  lcd.print("Bateria:       ");
  lcd.setCursor(9,0);
  lcd.print(Vin);
